	...
	//print_R($config);die;
		...
		public function create_product_category()
		{
			...
			if($this->form_validation->run() === FALSE){
				...
				$response1  = $this->Administrator_Model->create_product_category();
		...
		public function create_product($page = 'add-product')
		{
			...
			$this->form_validation->set_rules('save_price', 'Price', 'required');
...
public function getsubcategoriesbyid(){
	...
	$subcats = $this->Administrator_Model->getsubcategoriesbycid($catid);
	$html= '';
	 if(!empty($subcats)){
		$subcategory = $subcats[0]['sub_category'];
		$sub_explode = explode(',',$subcategory);
		$html.= '<select name="subcat_id" class="form-control form-control-primary">';
		foreach($sub_explode as $ros){
			$html.= '<option value="'.$ros.'">'.$ros.'</option>';
		}
		$html.= '</select>';
		echo $html;
	}
